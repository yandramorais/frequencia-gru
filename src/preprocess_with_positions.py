...
def bandpass_filter(x: np.ndarray, fs: float, low: float, high: float, order: int = 4):
...
def infer_fs(ts):
    """
    Infere frequência de amostragem a partir de timestamps.
    Robusto contra:
    - ts = None
    - ts escalar
    - ts object
...
def build_windows(dataset_path: Path, gt_dir: Path,
    ...
    for fp in files:
        ...
        X_filt = bandpass_filter(X_raw, fs_eff, *BANDPASS)
        X_filt = zscore(X_filt)

        window = int(window_sec * fs_eff)
        step = int(step_sec * fs_eff)

        gt_df = load_smartwatch_gt(gt_fp)
        Xw, yw = sliding_window_with_gt(X_filt, ts, gt_df, fs_eff, window, step)
        
        print("Arquivo:", fp.name)
        print("  GT:", gt_fp)
